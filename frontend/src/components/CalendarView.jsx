...
const CalendarView = ({ currentDate, learnings, onViewClick, selectedDate, onMonthChange, plans = [] }) => {
    ...
    const getEfficiencyColor = (efficiency) => {
        ...
        // Green: #34D800 = rgb(52, 216, 0) - 100% completion
            ...
            <div className="grid grid-cols-7 gap-2">
                {days.map((day, index) => {
                    ...
                    if (!day) {
                        ...
                        // All date blocks get default gray background
                        cellClasses += ' bg-gray-800/40 border-gray-700/30';

                        // Today highlight (blue ring)
                        if (isToday(day)) {
                            cellClasses += ' ring-2 ring-north-blue ring-offset-2 ring-offset-gray-900';
                        }
                                ...
                                <>
                                    <div className="flex justify-between items-start w-full">
                                        <span
                                            ...
                                            ${isToday(day) ? 'bg-north-blue text-white shadow-lg' :
                                    ...
                                    {/* LED Efficiency Indicator - Bottom Left */}
                                    {efficiency !== null && (
                                        <div
                                            className="w-1.5 h-1.5 rounded-full absolute bottom-3 left-3"
                                            style={{
                                                backgroundColor: getEfficiencyColor(efficiency),
                                                boxShadow: `
                                            0 0 6px ${getEfficiencyColor(efficiency)},
                                            0 0 4px ${getEfficiencyColor(efficiency)},
                                            0 0 2px ${getEfficiencyColor(efficiency)},
